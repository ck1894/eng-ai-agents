...
def group_times_into_segments(times: list[float], max_gap_sec: float) -> list[tuple[float, float]]:
    """
    Given sorted times (seconds), group into segments allowing gaps up to max_gap_sec.
...
def pick_segment_label_and_support(
    segment_frame_rows: list[int],
    query_set: set[str],
    frame_label_sets: list[set[str]],
) -> tuple[str, int]:
    """
    Pick ONE class label for the segment ("class_label used for retrieval"),
    and compute number_of_supporting_detections consistent with that label.

    Rule:
    - For each frame in the segment: overlap = query_set ∩ frame_labels
    - Count how often each overlapped label appears across frames
    - Choose the most frequent label (deterministic tie-break)
    - supporting detections = #frames in segment where chosen label appears
    """
    ...
    support = int(counts[best_label])
    return best_label, support
...
def main() -> None:
    ...
    parser.add_argument("--topk", type=int, default=None)  # keep top-k labels by confidence per query
    ...
    for cand in ["timestamp_sec", "timestamp", "time_sec"]:
        if cand in df_frames.columns:
            ...
            break
    if time_col is None:
        # fallback: use row index as time (works if 1 FPS and row order matches time)
    ...
    for qi in range(n_queries):
        ...
        if not results:
            if (qi % args.debug_every) == 0:
                print(f"[{qi:03d}] no yolo results")
            continue
        ...
        # Group matched times into segments
        ...
        for (start_t, end_t) in segments:
            # IMPORTANT: inclusive duration for 1 FPS (10..12 is 3 seconds)
            ...
            seg_label, support = pick_segment_label_and_support(seg_rows, query_set, frame_label_sets)
